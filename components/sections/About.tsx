...
const About: React.FC = () => {
  return (
    <section id="about" className="py-20 md:py-32 px-4 overflow-hidden relative">
      <div className="absolute top-1/2 left-0 -translate-y-1/2 text-[25vw] md:text-[20vw] font-black text-white/[0.02] whitespace-nowrap pointer-events-none select-none">
        ABOUT ME • ABOUT ME
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 md:gap-24 items-center">
          ...
          >
            ...
            <div className="space-y-6 md:space-y-8 text-base md:text-lg text-white/50 leading-relaxed font-light">
              ...
              <p>
                Como desenvolvedor assistido por IA, meu papel é orquestrar a inteligência das máquinas para potencializar a visão humana.
              </p>
              <p className="hidden md:block">
                Acredito na simplicidade que esconde a complexidade sob uma interface impecável. Minha jornada é marcada pelo aprendizado contínuo e pela busca da perfeição visual.
              </p>
            </div>

            <div className="mt-10 md:mt-12 flex gap-8 md:gap-12">
              <div>
                <p className="text-2xl md:text-3xl font-bold font-space text-white">1</p>
                <p className="text-[8px] md:text-[10px] uppercase tracking-widest text-white/30 font-bold">Ano de Exp.</p>
              </div>
              <div>
                <p className="text-2xl md:text-3xl font-bold font-space text-white">10+</p>
                <p className="text-[8px] md:text-[10px] uppercase tracking-widest text-white/30 font-bold">Projetos</p>
              </div>
              <div>
                <p className="text-2xl md:text-3xl font-bold font-space text-white">100%</p>
                <p className="text-[8px] md:text-[10px] uppercase tracking-widest text-white/30 font-bold">Foco</p>
              </div>
            </div>
          </motion.div>
            ...
            <div className="relative aspect-[4/5] sm:aspect-square md:aspect-[4/5] rounded-[2rem] md:rounded-[2.5rem] overflow-hidden border border-white/10 p-1 md:p-2 bg-gradient-to-br from-white/15 to-transparent shadow-2xl transition-all duration-700">
              <div className="w-full h-full rounded-[1.8rem] md:rounded-[2rem] overflow-hidden relative bg-black">
                ...
                <div className="absolute bottom-6 left-6 md:bottom-10 md:left-10 z-20">
                  ...
                  <p className="text-cyan-400 uppercase tracking-[0.2em] text-[8px] md:text-[10px] font-bold mt-1">AI-Driven Architect</p>
                </div>
              </div>
            </div>

            {/* AI Badge ajustado para mobile */}
            <motion.div 
              animate={{ y: [0, -10, 0] }}
              transition={{ duration: 5, repeat: Infinity }}
              className="absolute -top-6 -right-6 md:-top-12 md:-right-12 w-32 h-32 md:w-48 md:h-48 glass-card rounded-[1.5rem] md:rounded-[2.5rem] flex items-center justify-center p-4 md:p-8 border border-cyan-400/40 shadow-xl z-20 backdrop-blur-3xl"
            >
              <div className="text-center">
                ...
                <p className="text-[7px] md:text-[10px] uppercase tracking-[0.1em] text-cyan-400/80 font-black">Focused Dev</p>
              </div>
            </motion.div>
          </motion.div>
        </div>
